...
public class ConcreteCodePatternFinder {
	...
	private ArrayList<CodePatternSet> removeSubset(ArrayList<CodePatternSet> sets){		
		Subset[] removeIdx = new Subset[sets.size()];				
	...
	private Subset subset(CodePatternSet set1, CodePatternSet set2) {
		...
		if(set1Size > set2Size) {
			...
			if(set1Size == set2Size && cnt == set1Size)
				return Subset.Dup;
			else if(cnt == set1Size) 
				return Subset.Yes;
			else 
				return Subset.No;
		}			
	...
	private ArrayList<CodePatternSet> removeInSet(ArrayList<CodePatternSet> sets){
	...
	public void writeConcreteCodePattern(ArrayList<CodePatternSet> sets) {
		...
		try(			
			...
			CSVPrinter csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT
					.withHeader("Pattern ID", "Pattern", "Low Frequency", "Pattern Cnt"));
			) {
			int cnt = 0;
			...
			for(int i = 0 ; i < sets.size(); i ++) {
				...
				for(Integer h : sets.get(i).getOnlyPatternHash()) {									
					v.put(h, 0);				
				}
				
				csvPrinter.printRecord(patternID, pattern, f, "" + v.size());				
			}
